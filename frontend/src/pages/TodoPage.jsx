...
import {
  ...
  List, ChevronLeft, ChevronRight, ChevronDown, CalendarDays, ListChecks,
} from "lucide-react";
...
function TodoRow({
  ...
  draggable, onDragStart, onDragOver, onDrop, overdue, t, compact = false,
}) {
  ...
  const handleSaveDetail = () => {
    onSaveDetails(draftStartTime || null, draftMemo.trim() || null);
  };
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 ${todo.is_done ? "opacity-60" : ""}`}>
      ...
      >
        ...
        {!editing && (
          <button onClick={toggleDetail} className="shrink-0" title={t("todo.detailToggle")}>
            <ChevronDown
              size={14}
              className={`transition-transform ${detailOpen ? "rotate-180" : ""} ${
                hasDetail ? "text-blue-400 dark:text-blue-400" : "text-gray-300 dark:text-gray-600"
              }`}
            />
          </button>
        )}
        ...
        {/* 마감일은 항상 행의 오른쪽 끝(수정/삭제 아이콘 바로 앞)에 정렬 */}
        {!editing && todo.due_date && (
      ...
      {detailOpen && !editing && (
        <div className="px-4 pb-3 pt-0 flex flex-col gap-2 border-t border-gray-50 dark:border-gray-700/60 mt-0.5">
          ...
          <button
            ...
            className="self-end flex items-center gap-1 text-xs bg-blue-600 text-white font-medium px-3 py-1.5 rounded-lg hover:bg-blue-700"
          >
...
function TodoPage() {
  ...
  const handleAdd = async () => {
    ...
    setError("");
    try {
      const created = await createTodo(newTitle.trim(), newDueDate || null, newPriority, token);
  ...
  const rowProps = (todo, draggable, compact = false) => ({
    todo,
              ...
              <div className="pt-3 border-t border-gray-100 dark:border-gray-700 flex flex-col gap-2 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="flex-1 text-sm border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-2 py-1.5 focus:outline-none"
                  >
                    {PRIORITIES.map((p) => (
                      <option key={p.value} value={p.value}>{t(p.labelKey)}</option>
                    ))}
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 bg-blue-600 text-white text-sm font-medium px-3 py-1.5 rounded-lg hover:bg-blue-700 disabled:opacity-50 shrink-0"
                  >
